...
class InMemoryTaskManagerTest {
    ...
    void init() {
        ...
        task = new Task("Task one", "Do something", Status.NEW);
        epic = new Epic("Epic one", "Do something");
    }

    // Объект Epic невозможно добавить в самого себя в виде подзадачи на этапе компиляции кода
    // Объект Subtask невозможно сделать своим же эпиком на этапе компиляции кода

    @Test
    void shouldAddNewTask() {
        // prepare
        Task expectedTask = new Task(1, "Task one", "Do something", Status.NEW);
    ...
    public void tasksWithGeneratedAndAssignedIdsDoNotConflict() {
        // prepare
        Task taskOne = new Task(1, "Task one", "Do something", Status.NEW);
        Task taskTwo = new Task("Task two", "Do something else", Status.IN_PROGRESS);
    ...
    void shouldUpdateTask() {
        // prepare
        Task expectedTask = new Task(1, "Task one", "Do something", Status.IN_PROGRESS);

        // do
        Task addedTask = taskManager.addNewTask(task);
        Task updatedTask = new Task(addedTask.getId(), "Task one", "Do something updated", Status.IN_PROGRESS);
    ...
    void shouldGetAllTasks() {
        ...
        Task taskTwo = new Task("Task two", "Do something else", Status.IN_PROGRESS);
        Task taskThree = new Task("Task three", "Do another thing", Status.DONE);
